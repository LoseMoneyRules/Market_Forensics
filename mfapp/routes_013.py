...
if ("historical-test", "Historical Test") not in SECTIONS:
    valuation_index = next((i for i, row in enumerate(SECTIONS) if row[0] == "valuation"), 3)
    SECTIONS.insert(valuation_index + 1, ("historical-test", "Historical Test"))
...
def save_valuation_013(ticker):
    ...
    model.calculation_version = "0.1.3"
...
@role_required("CONTROL")
def reset_valuation_013(ticker):
    require_control_view(); ctx = _ctx(ticker); result = prefill_coverage(ctx["coverage"].id, g.user.id, force=True)
    audit("valuation.model.reset", "coverage", ctx["coverage"].id, {"engine": "0.1.3"}); db.session.commit()
...
@role_required("CONTROL")
def financial_flows_013(ticker):
    ...
    return render_template("financial_flows_013.html", periods=periods, selected_year=requested, flows=flows, **ctx)


@bp.get("/company/<ticker>/historical-test")
@role_required("CONTROL")
def historical_test_013(ticker):
    ...
    return render_template("historical_test_013.html", runs=runs, latest_run=latest, samples=samples, **ctx)


@bp.post("/company/<ticker>/historical-test/run")
@role_required("CONTROL")
def queue_historical_test_013(ticker):
    require_control_view(); ctx = _ctx(ticker); lookback = max(3, min(int(n(request.form.get("lookback_years")) or 10), 15))
    ...
    return redirect(url_for("web.historical_test_013", ticker=ticker.upper()))
