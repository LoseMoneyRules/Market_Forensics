...
def _alpaca_quote(ticker: str, user_id: int) -> QuoteResult:
    ...
    try:
        ...
        return QuoteResult(True, "Alpaca IEX", float(price), "USD", _utc_naive(trade.get("t") or daily.get("t")), "LIVE/OBSERVED", {"source": "snapshot", "feed": "iex"})
    except Exception as exc:
        return QuoteResult(False, "Alpaca", message=type(exc).__name__)


def _tiingo_quote(ticker: str, user_id: int) -> QuoteResult:
    ...
    try:
        r = requests.get(
            ...
            params={"token": token},
            headers={"User-Agent": "MarketForensics/0.0.2"},
            timeout=8,
        )
        if r.status_code != 200:
        ...
        return QuoteResult(True, "Tiingo", float(price), "USD", _utc_naive(row.get("timestamp") or row.get("quoteTimestamp")), "OBSERVED", {"source": "iex"})
    except Exception as exc:
        return QuoteResult(False, "Tiingo", message=type(exc).__name__)


def _alpha_vantage_quote(ticker: str, user_id: int) -> QuoteResult:
    ...
    try:
        ...
        return QuoteResult(True, "Alpha Vantage", float(price), "USD", _utc_naive(), "DELAYED/OBSERVED", {"source": "global_quote"})
    except Exception as exc:
        return QuoteResult(False, "Alpha Vantage", message=type(exc).__name__)


def _yahoo_quote(ticker: str) -> QuoteResult:
    """Public market-chart fallback used only after the configured V3.1.12 provider chain."""
    ...
    try:
        ...
        return QuoteResult(True, "Public market chart", float(price), str(meta.get("currency") or "USD"), as_of, "PUBLIC FALLBACK", {"exchange": meta.get("exchangeName") or "", "instrument_type": meta.get("instrumentType") or "", "previous_close": meta.get("chartPreviousClose")})
    except Exception as exc:
...
def refresh_quote(company_id: int, ticker: str, user_id: int) -> QuoteResult:
    """V3.1.12-style resilient quote chain for the hosted app.

    Hosted Namecheap cannot run a local IBKR Client Portal Gateway, so that desktop-only hop is
    intentionally skipped. Order: Alpaca -> Tiingo -> Alpha Vantage -> public market-chart fallback.
    If every live source fails, the existing MarketSnapshot remains untouched as last-good cache.
    """
    attempts = [
        _alpaca_quote(ticker, user_id),
        _tiingo_quote(ticker, user_id),
        _alpha_vantage_quote(ticker, user_id),
        _yahoo_quote(ticker),
    ]
    ...
    if result is None:
        ...
        return QuoteResult(False, "last-good cache", message=messages)
