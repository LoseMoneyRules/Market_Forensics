...
def create_invite():
    ...
    inv = Invite(email=email, role=role, token_hash=hashlib.sha256(raw.encode()).hexdigest(), expires_at=now_utc() + timedelta(days=7), created_by=g.user.id)
    db.session.add(inv)
...
def add_company():
    ...
    if not re.fullmatch(r"[A-Z0-9][A-Z0-9.\-]{0,11}", ticker):
        flash("Ticker not found / symbol not recognized.", "error")
        return redirect(url_for("web.control"))
    if not name:
        flash("Company name is required in v0.0.1.", "error")
        return redirect(url_for("web.control"))
    ...
    db.session.add(c)
    db.session.flush()
    audit("company.create", "company", c.id, {"ticker": ticker})
    db.session.commit()
