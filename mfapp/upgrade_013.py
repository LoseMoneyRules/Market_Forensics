...
MIGRATION_KEY = "release_0_1_3_queue_intrinsic_valuation_prefill"
...
def queue_existing_coverage_prefill() -> dict[str, Any]:
    ...
    for coverage in Coverage.query.order_by(Coverage.id.asc()).all():
        ...
        if security is None:
            skipped += 1; continue
        existing = Job.query.filter(Job.user_id == coverage.user_id, Job.job_type == "RESEARCH_PREFILL", Job.security_id == security.id, Job.status.in_(ACTIVE_STATUSES)).order_by(Job.id.asc()).first()
        if existing is not None:
            reused += 1; continue
        db.session.add(Job(job_type="RESEARCH_PREFILL", status="QUEUED", priority=75, user_id=coverage.user_id,
                           company_id=security.company_id, security_id=security.id,
                           payload={"coverage_id": coverage.id, "upgrade": "0.1.3"}, run_after=now))
        queued += 1
