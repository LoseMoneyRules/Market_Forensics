...
from .insights_016 import audit_2_summary, build_synthesis, expectations_chart, valuation_price_history
from .monitoring_016 import NUMERIC_OPERATORS
...
@bp.route("/alerts/017/subscription/<int:coverage_id>", methods=["GET", "POST"])
@login_required
def alert_subscription_017(coverage_id: int):
    try:
...
@bp.post("/company/<ticker>/alerts/017/catalog")
@role_required("CONTROL")
def alert_catalog_017(ticker: str):
    require_control_view()
    ctx = _ctx(ticker)
    payload = request.get_json(silent=True) or {}
    try:
...
def alert_policy_017(ticker: str):
    """Backward-compatible CONTROL subscription endpoint used by early 0.1.7 UI work."""
    require_control_view()
    ctx = _ctx(ticker)
    payload = request.get_json(silent=True) or {}
    value = save_alert_subscription(
...
@bp.route("/company/<ticker>/alerts/017/rule/<int:rule_id>", methods=["PATCH", "DELETE"])
@role_required("CONTROL")
def alert_rule_017(ticker: str, rule_id: int):
    require_control_view()
    ctx = _ctx(ticker)
