...
from .flows_016 import FLOW_VERSION, build_cash_flow, build_income_statement_flow
from .insights_016 import (
    audit_2_summary,
    build_synthesis,
...
def _store_flow(period: FinancialPeriod, flow_type: str, payload: dict) -> FinancialFlow:
    ...
    if existing is None:
        db.session.add(Provenance(
            ...
            calculation_version=FLOW_VERSION,
            notes="0.1.6 explicit accounting bridge; node values are statement subtotals, edges are reconciled uses/contributions.",
        ))
    return row
...
@bp.get("/company/<ticker>/surface/016/<section>")
@role_required("CONTROL")
def surface_016(ticker: str, section: str):
    require_control_view(); ctx = _ctx(ticker)
    company = ctx["company"]; security = ctx["security"]; coverage = ctx["coverage"]
    base = {"version": "0.1.6", "section": section, "ticker": security.ticker}
...
@bp.post("/company/<ticker>/financial-flows/recalculate-016")
@role_required("CONTROL")
def recalculate_flows_016(ticker: str):
    ...
    audit("financial_flows.recalculate_016", "company", ctx["company"].id, {"rows": count, "calculation_version": FLOW_VERSION})
    db.session.commit()
    flash(f"Financial Flows rebuilt with 0.1.6 accounting bridge ({count} flow rows).", "success")
    return redirect(url_for("web.financial_flows_013", ticker=ticker.upper()))


@bp.post("/company/<ticker>/monitoring/evaluate-016")
@role_required("CONTROL")
def evaluate_monitoring_016(ticker: str):
    require_control_view(); ctx = _ctx(ticker)
    result = evaluate_coverage(ctx["coverage"].id, g.user.id)
    audit("monitoring.evaluate_016", "coverage", ctx["coverage"].id, {"created_alerts": result.get("created_alerts", 0)})
    db.session.commit()
...
@bp.route("/settings/notifications/016", methods=["GET", "POST"])
@role_required("CONTROL")
def notification_settings_016():
