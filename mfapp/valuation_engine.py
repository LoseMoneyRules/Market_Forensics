...
def metrics_from_history(history: list[dict[str, Any]], shares_override: Any = None, share_source: str = "") -> dict[str, Any]:
    ...
    if not rows:
        return {"basis_usable": False, "basis_issue": "No annual filing-derived fundamentals are available."}
    latest = rows[-1]
    ...
    net_margins = [(n(x.get("net_income")) / n(x.get("revenue"))) if n(x.get("net_income")) is not None and n(x.get("revenue")) not in (None, 0) else None for x in rows]
    fcf_margins = [(n(x.get("fcf")) / n(x.get("revenue"))) if n(x.get("fcf")) is not None and n(x.get("revenue")) not in (None, 0) else None for x in rows]
    ...
    return {
        ...
        "basis_issue": "" if shares not in (None, 0) else "No usable shares outstanding / diluted-share denominator is available.",
    }
...
def calibrate_multiples(observations: list[dict[str, Any]], company_type: str) -> dict[str, Any]:
    ...
    Each observation must already obey the as-of boundary and contain price, revenue,
    net income, FCF, net debt and shares on one contemporaneous share basis.
    """
    ...
    for row in observations:
        ...
        revenue, ni, fcf, net_debt = n(row.get("revenue")), n(row.get("net_income")), n(row.get("fcf")), n(row.get("net_debt")) or 0.0
    ...
    pe = (quantile(pe_values, .25) or prior["pe"][0], median(pe_values) if pe_values else prior["pe"][1], quantile(pe_values, .75) or prior["pe"][2])
    evs = (quantile(evs_values, .25) or prior["ev_sales"][0], median(evs_values) if evs_values else prior["ev_sales"][1], quantile(evs_values, .75) or prior["ev_sales"][2])
    # A lower FCF yield means a higher valuation, so bull/base/bear ordering is reversed.
    fy = (quantile(fcf_yields, .75) or prior["fcf_yield"][0], median(fcf_yields) if fcf_yields else prior["fcf_yield"][1], quantile(fcf_yields, .25) or prior["fcf_yield"][2])
    return {"source": "POINT_IN_TIME_CALIBRATION", "sample_size": max(len(pe_values), len(evs_values), len(fcf_yields)), "pe": pe, "ev_sales": evs, "fcf_yield": fy}


def default_cases(metrics: dict[str, Any], company_type: str = "Generic", calibration: dict[str, Any] | None = None) -> dict[str, Any]:
    calibration = calibration or {"source": "TYPE_PRIOR", **TYPE_PRIORS.get(company_type, TYPE_PRIORS["Generic"])}
    ...
    if fm is None:
        fm = .07
    pe = calibration.get("pe") or TYPE_PRIORS[company_type]["pe"]
    evs = calibration.get("ev_sales") or TYPE_PRIORS[company_type]["ev_sales"]
    fy = calibration.get("fcf_yield") or TYPE_PRIORS[company_type]["fcf_yield"]
...
def scenario_value(metrics: dict[str, Any], assumptions: dict[str, Any], weights: dict[str, Any], years: int = 5) -> dict[str, Any]:
    if not metrics.get("basis_usable"):
        issue = metrics.get("basis_issue") or "Share basis is unavailable."
        return {"fair_value": None, "pe": None, "ev_sales": None, "fcf_yield": None, "dcf": None, "effective_weights": {}, "flags": [f"DATA REVIEW: {issue}"]}
    ...
    components = {"pe": pe, "ev_sales": evs, "fcf_yield": fcf}
    fair, effective, flags = robust_blend(components, weights)
    ...
    return {
        ...
        "fcf_yield": fcf,
        "dcf": dcf,
        "range_low": quantile(valid, .25) if valid else None,
        "range_high": quantile(valid, .75) if valid else None,
        "effective_weights": effective,
        "flags": flags,
    }


def evaluate(metrics: dict[str, Any], cases: dict[str, dict[str, Any]], weights: dict[str, Any], years: int = 5, current_price: Any = None) -> dict[str, Any]:
    scenarios: dict[str, dict[str, Any]] = {}
    for name in ("BEAR", "BASE", "BULL"):
        row = scenario_value(metrics, cases.get(name) or {}, weights, years)
    ...
    return {"scenarios": scenarios, "expected_value": expected, "current_price": n(current_price), "engine_version": ENGINE_VERSION}
