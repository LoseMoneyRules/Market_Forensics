...
from mfapp.core_models import Company, Coverage, FinancialPeriod, MarketSnapshot, NormalizedFinancial, Security, ValuationModel, ValuationScenario
...
def build_app(tmp_path, monkeypatch):
    ...
    return create_app({
        "TESTING": True,
        "SECRET_KEY": "012-test",
        "SQLALCHEMY_DATABASE_URI": f"sqlite:///{tmp_path / 'v012.db'}",
...
def seed_company(app):
    with app.app_context():
        user = User(email="control012@example.com", display_name="Control", role="CONTROL",
        ...
        security = Security(company_id=company.id, ticker="EXM", exchange="NYSE", currency="USD",
        ...
        for fy, revenue, net_income, fcf, shares in values:
            period = FinancialPeriod(company_id=company.id, period_type="FY", fiscal_year=fy,
                                     start_date=date(fy - 1, 7, 1), end_date=date(fy, 6, 30), currency="USD")
            db.session.add(period); db.session.flush()
            db.session.add(NormalizedFinancial(
                financial_period_id=period.id,
                revenue=Decimal(revenue), gross_profit=Decimal(revenue) * Decimal("0.44"),
                operating_income=Decimal(revenue) * Decimal("0.12"), net_income=Decimal(net_income),
                cfo=Decimal(fcf) + Decimal(300_000_000), capex=Decimal(300_000_000), fcf=Decimal(fcf),
                inventory=Decimal(2_000_000_000 + (fy - 2023) * 50_000_000),
                receivables=Decimal(1_100_000_000), payables=Decimal(900_000_000),
                cash=Decimal(2_000_000_000), debt=Decimal(1_200_000_000), equity=Decimal(5_000_000_000),
                diluted_shares=Decimal(shares), shares_outstanding=Decimal(shares), source_map={}, quality={"test": True},
            ))
        db.session.add(MarketSnapshot(security_id=security.id, provider="TEST", price=Decimal("37.50"), currency="USD",
                                      as_of=datetime(2026, 9, 17, 12, 0), quality="OBSERVED", payload={}))
        db.session.commit()
...
def test_display_preference_persists_per_user(tmp_path, monkeypatch):
    ...
    user_id, _ = seed_company(app)
    client = app.test_client(); login(client, user_id)
    response = client.post("/settings/display", data={"number_format": "B"}, follow_redirects=False)
    assert response.status_code in (302, 303)
    with app.app_context():
        row = UserPreference.query.filter_by(user_id=user_id, key="number_format").first()
        assert row is not None
        assert row.value["mode"] == "B"
...
def test_autofill_builds_coherent_scenarios_and_preserves_manual_override(tmp_path, monkeypatch):
    ...
    with app.app_context():
        ...
        scenarios["BASE"].inputs = {"manual_value": 77.77}
        db.session.commit()
        second = prefill_coverage(coverage_id, user_id)
        db.session.refresh(scenarios["BASE"])
...
def test_finra_public_daily_requires_no_credential_and_api_status_is_optional(tmp_path, monkeypatch):
    ...
    with app.app_context():
        ...
        assert status["finra"] is True
        assert status["finra_api"] is False
        set_secret(user_id, "finra_client_id", "public-client")
        set_secret(user_id, "finra_client_secret", "public-secret")
        db.session.commit()
        assert provider_status(user_id)["finra_api"] is True
    ...
    rows = daily_short_volume("EXM", lookback_days=1)
    assert rows
    assert rows[-1]["short_volume"] == 300.0
    assert rows[-1]["short_pct"] == 0.32
