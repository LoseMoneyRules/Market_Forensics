...
def test_primary_get_routes_render_without_mutating_research_state(tmp_path, monkeypatch):
    ...
    for path in ("/", "/discovery", "/company/NKE/overview", "/company/NKE/valuation", "/company/NKE/historical-test", "/company/NKE/financial-flows", "/company/NKE/tape", "/portfolio", "/publications", "/settings"):
...
def test_company_publish_entrypoint_and_settings_controls_are_visible(tmp_path, monkeypatch):
    app = build_app(tmp_path, monkeypatch); user_id = seed_control(app); client = app.test_client(); login_session(client, user_id)
    overview = client.get("/company/NKE/overview").get_data(as_text=True); assert "Publish research" in overview; assert "Review before publish" in overview
...
def test_health_is_public_and_identifies_web_native_release(tmp_path, monkeypatch):
    ...
    payload = response.get_json(); assert payload["version"] == "0.1.3"; assert payload["architecture"] == "web-native"
