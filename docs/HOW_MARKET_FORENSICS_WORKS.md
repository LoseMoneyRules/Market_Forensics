...
A 10-K can contain cover-page instant facts measured after the fiscal balance-sheet date. Those instants remain valid source evidence but do not define a new fiscal year. Canonical annual financial periods require annual duration-statement evidence, and annual instant facts used in an FY basis must fall within the fiscal-year-end tolerance. Instant-only legacy FY shells are excluded from annual history/current-basis selection rather than being allowed to hide the latest complete filed FY.
...
**Permanent fiscal-calendar integrity rule:** represented fiscal periods come from the fact's own dates, but the issuer's current SEC fiscal-year-end MMDD is not an exact historical cutover for 52/53-week calendars. Period ends within a narrow year-end tolerance remain in that represented fiscal year. A parser revision that changes a stored fiscal-year/quarter label must repair the existing same-end-date period in place rather than leave a duplicate or a fake missing year. A real SEC-filed FY such as a 52/53-week Apple year may never disappear because the nominal fiscal-year-end day drifted by a few days.

**Permanent current-basis rule:** Current Financial Anatomy, the Fundamentals current strip, Expectations and any valuation input that consumes the current row must use the freshest filed economic basis by period end. A reconstructable TTM may not outrank a newer filed FY. At the same fiscal year-end date, the filed FY is preferred because its duration is equivalent to that fiscal-year TTM while its audited balance-sheet instants are generally more complete. A newer post-FY TTM remains current when its end date is later.

**Permanent user-facing version-hygiene rule:** product pages, cards, notices, charts and report prose must never hardcode historical release numbers such as 0.x.y. If the current product version genuinely needs to be shown, it must come from the canonical runtime VERSION value. Internal engine revisions may exist for cache/provenance purposes, but they must use engine identifiers or remain in audit metadata and must not leak obsolete product-release labels into normal user-facing Research pages.
