...
**0.3.2 Valuation/Validate/Peer Integrity branch:** `fix/0.3.2-valuation-validate-peer-integrity`, created directly from clean current main `4056482cce9f6f15c721464ea547c4d6cdbdfb5d`; VERSION remains 0.3.2. Engine version becomes `0.3.2-integrity-v2` so pre-regime cached auto valuation cannot masquerade as current.  
**0.3.2 regime-aware history rule:** automatic operating distributions, historical multiple calibration and valuation-forensics anchors detect only persistent multi-dimensional structural business breaks; pre-regime fiscal years are excluded from current-regime medians. A one-year shock cannot reset history. If the post-change regime lacks enough comparable observations, the affected method remains unavailable/review rather than borrowing a structurally different old multiple.  
**0.3.2 Validate parity rule:** Historical Validate uses the same canonical valuation engine/version, company-specific point-in-time calibration, EV/EBITDA/equity evidence and regime boundary as live Valuation. Reference-price fallback is disabled. Only historical snapshots with INTRINSIC Base and at least two independent valuation families can contribute to reliability/VALIDATED; other samples are MODEL_LIMITED. Regime detection is point-in-time with no future leakage. Any prior Validate run from a different valuation-engine version is automatically REVIEW/STALE until rerun; an obsolete validation score cannot support current Model Confidence.  
**0.3.2 peer integrity rule:** presence in the user's Research database never qualifies a company as a valuation peer. CLOSE/PARTIAL eligibility requires structural comparability (same SIC, SIC division or industry) plus economic similarity; same sector/numerical resemblance alone is REFERENCE ONLY. Peer-adjusted valuation requires at least three structurally comparable peers, otherwise it fails closed. The peer candidate universe is still limited to stored normalized companies/quotes and that limitation is explicit.  
