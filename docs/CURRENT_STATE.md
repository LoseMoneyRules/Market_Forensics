...
**0.3.7 branch:** `release/0.3.7-ui-restoration`, created directly from production-synced clean `main` `0d93894e4c3bcacc4086af69612c244a1615a806`.  
**0.3.7 scope:** UI/UX restoration and responsive consolidation only. Preserve all 0.3.6 investment logic, data/provider behavior, Research/Validation/Portfolio separation, jobs, database, authentication/security, reports and deployment architecture. The release re-establishes one coherent visual contract across desktop and iPhone rather than layering page-specific fixes.  
**0.3.7 responsive rule:** mobile respects iOS safe-area insets, page-level horizontal overflow is contained while intentionally wide tables remain independently scrollable, the fixed topbar/tools/navigation share one height contract, dense KPI/financial strips remain readable on iPhone, and research/navigation controls retain 44px touch targets. Desktop retains the institutional 14px typography, compact hierarchy, minimal bold and bounded table density.  
**0.3.7 pull request / merge:** PR #76 `0.3.7: UI restoration and responsive consolidation`; final branch CI run `35800431197` / #1955 SUCCESS and final PR CI run `35800435593` / #1956 SUCCESS; squash-merged to `main` as `82eaf119bb87f2402b8485a13c8692b7f05f60bd`.  
**0.3.7 post-merge verification:** main CI run `35800571859` / #1957 SUCCESS; release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production remains 0.3.6 until an explicit manual deploy.  
**0.3.7 interaction closure:** mobile micro-actions, tabs and company navigation use real 44px hit areas; phone form controls use a 16px input font to prevent Safari focus zoom; auth/flash chrome respects safe areas; stacked Process Readiness and Discovery text remains visible instead of being clipped or ellipsized.  
**0.3.7 desktop closure:** the 821–1100px header compresses nonessential labels before they collide, while normal desktop typography and the canonical navigation architecture remain unchanged.  
**0.3.7 responsive-density R2:** follow-up UI audit from production-synced `main` `12f9ed7669f39d1dfa449c8f40c92562aff212c7`. Discovery summary chips with long dynamic Stage labels wrap inside the panel instead of forcing page overflow. Two/three-column source, short-interest, Portfolio factor/correlation and provider tables are explicitly compact and no longer inherit the generic 760px evidence-table minimum; genuinely wide evidence tables retain their local horizontal-scroll contracts. VERSION remains 0.3.7 and no analytical/data/security behavior changes.   The mobile `•••` tools control is independently right-anchored, so showing the sticky ticker/price context while scrolling cannot shift the control.
**0.3.7 responsive-density R2 merge / verification:** PR #78 `0.3.7: Fix Discovery overflow and compact tables` merged by squash to `main` as `df131e461304059cab284a312fff1415be901854`; post-merge main CI run `35807716014` / #1970 completed SUCCESS. Release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke passed. Production still runs the earlier 0.3.7 source `3a3eb1fb8009f98ff26d31a71dc75a4d1050fccf` until this same-version UI fix is explicitly deployed.  
**0.3.6 branch:** `release/0.3.6-integrity-consolidation-r2`, created directly from current `main` `9bea092b2bbc3e0c3acd7a6fcfe7ac83c071c469`. This is the only 0.3.6 release branch intended for merge. It supersedes the parallel unmerged 0.3.5 attempt and the earlier pre-sync 0.3.6 branch.  
**0.3.6 financial evidence rule:** live FY/Q reads keep one durable period identity for writes/audit but coalesce normalized fields across exact-same-end-date active / `SUPERSEDED_*` / `SUP_*` siblings. Recovered fields retain explicit `SAME_PERIOD_EVIDENCE_COALESCE` provenance and source period id; same-period Economic Reality may also be recovered. No value is carried across dates. Current Financial Anatomy, the Fundamentals current strip, recalculation/Financial Flows and downstream Research therefore consume the same represented-period evidence.  
**0.3.6 current-basis completeness rule:** a reconstructed TTM may replace the latest filed FY only when it preserves every material decision-surface field that the latest FY proves applicable. A newer Revenue-only/sparse TTM is withheld rather than blanking Current Financial Anatomy, the Fundamentals current strip, forecasts, completeness and downstream Research. The system never carries FY values across dates; it keeps the complete FY canonical until the newer quarterly basis can stand on its own evidence.  
**0.3.6 fiscal-instant integrity rule:** a 10-K cover-page instant dated after fiscal year-end (for example `EntityCommonStockSharesOutstanding` measured at the filing-cover date) is not an annual financial period and may not create a next-year FY shell. Annual instant facts must be within the fiscal-year-end tolerance; legacy FY identities containing only instant facts are excluded from canonical annual reads/current basis. SEC normalizer revision is `0.3.6-fiscal-instant-integrity-r3`, forcing stale Coverage through normal refresh.  
**0.3.6 INTU/current-surface closure:** PR #74 `0.3.6: Prevent sparse TTM from blanking current fundamentals` passed branch CI `35790090827` and PR CI `35790245908`, merged by squash to `main` as `b47ce556f41fb749e29900cfeba538fb0984882d`, and post-merge main CI `35790385873` completed SUCCESS. The fix is in main but is not in the currently running 0.3.6 production source until the next manual deploy.  
**0.3.6 Research cache rule:** `RECALCULATE` is the single canonical builder of `RESEARCH_CACHE`. Provider/evidence jobs persist evidence and queue recalculation; they do not partially mutate the Research cache. Normal GET navigation makes no provider calls. Tape may rebuild only its display surface from fresher materialized DB evidence while the canonical cache is stale/pending.  
**0.3.6 Tape rule:** an explicit Tape / Positioning refresh also queues the HistoricalPrice spine required by Tape charts. Valid reconciled consolidated-SIP observations remain visible when direction-eligible coverage is low, but low coverage is a decision/scoring restriction and cannot move Tape rank/regime. Partial samples remain context-only. Flow method version is `0.3.6-reconciled-observation-v2`.  
**0.3.5 status:** PR #70 `0.3.5: Data surface coherence and Tape recovery` is superseded by 0.3.6 and must not be merged. Its valid ideas were re-applied selectively on top of current `main`.  
**0.3.6 pull request / merge:** PR #72 `0.3.6: Consolidate financial current-basis and Tape integrity` passed branch CI run `35782717998` and PR CI run `35782879956`, then merged by squash to `main` as `343b3f30d29a8a38e07b8a58e15a30b5a83901e6`.  
**0.3.6 post-merge verification:** main CI run `35783040423` SUCCESS at `343b3f30d29a8a38e07b8a58e15a30b5a83901e6`; release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Manual deploy run `35783546157` / deploy #70 subsequently advanced Namecheap to 0.3.6 and passed candidate plus post-cleanup `/health`; only the final documentation state-sync step failed.  
**0.3.6 deploy-state repair:** keep exactly one top-level `**Production:**` line. Deploy preflight now enforces that invariant before any upload, so a malformed CURRENT_STATE cannot turn an otherwise healthy deployment red after production has already been swapped.  
**Production:** 0.3.7 on Namecheap; verified production health after manual deploy run `35808743749` / deploy #74 from main source `75dc34d952bd7bca24e4a45e5622ecdc837ce3a4` at 2026-09-23T02:06:23Z; /health matched VERSION and returned `architecture=web-native`, `database=primary`, `reports=rich`, `status=ok`.  
**0.3.4 integrity-closure branch:** `fix/0.3.4-integrity-closure`, created directly from production-synced 0.3.4 main `6461fcd0579c9a8626a2749649a9cc6cd38206c2`. VERSION remains 0.3.4. This pass closes the remaining financial-current and Tape materialization gaps structurally; it is not a ticker-specific patch and is not patch-on-patch.  
**0.3.4 represented-period coalescing rule:** one period identity remains the durable write/audit target, but live FY/Q read surfaces coalesce valid normalized fields across every same-end active / `SUPERSEDED_*` / `SUP_*` family member field-by-field. Revenue, Inventory, Cash, Receivables and other facts therefore cannot disappear merely because complementary evidence lives on a sibling identity. Current Financial Anatomy, the Fundamentals current strip, Financial Flows/recalculation, readiness, reports and valuation consume the same coalesced basis. SEC normalizer `0.3.4-data-surface-integrity-r2` forces existing Coverage through the supported stale-refresh self-heal path. `current_row()` also retains the prior FY comparator for derived growth metrics.  
**0.3.4 Tape materialization rule:** Large/Whale remains a dollar-notional trade-size proxy. Positioning, FINRA and Price History background jobs publish newly stored Tape evidence into the latest Research cache immediately and still queue the normal full recalculation. Normal Tape GET navigation makes no provider calls; when the Research cache is absent or older than stored Tape/FINRA evidence, only the Tape surface is rebuilt from local DB evidence so `panel tape-v2` and charts do not render blank. Newer stored Tape evidence also makes the Research cache stale. Partial/page-capped reconciled SIP samples may be displayed as context but never drive Tape rank/regime; only complete integrity-passing consolidated-SIP observations may score.  
**0.3.4 original data-surface PR:** PR #68 `0.3.4: Financial & Tape data-surface integrity` passed branch CI #1828 / run `35758056190` and PR CI #1829 / run `35758130036`, then merged as `a154f45276363a60e658337b7f486146190956f6`. Post-merge CI #1830 / run `35758299401` passed. The later production deploy succeeded; this integrity-closure branch starts from that deployed state.  
**0.3.4 integrity-closure pull request / merge:** PR #71 `0.3.4: Close financial current-surface and Tape materialization gaps` passed branch CI #1888 / run `35779575156` at `dde45e84a42451efd37dfcac6869921009ef1866` and PR CI #1890 / run `35779804724` at `2e8f027ac2db749c8830051d8d6d9da18c9cf0ec`, then merged by squash to `main` as `361a08a31d8319424aead22c1b156f1fbb889ae3`.  
**0.3.4 integrity-closure verification:** post-merge main CI #1891 / run `35779985884` SUCCESS at `361a08a31d8319424aead22c1b156f1fbb889ae3`. Release metadata, Python/YAML/JavaScript checks, full pytest (including populated Fundamentals/Tape rendering), production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production still requires a new manual deploy.  
**0.3.3 production-data-truth branch:** `fix/0.3.3-production-data-truth`, created directly from production-synced 0.3.3 main `e22e241af849e0d2fc26ddd96bba7bbf548c1283`; VERSION remains 0.3.3. This pass addresses production-only legacy FinancialPeriod duplication that synthetic parser tests did not reproduce, and makes Tape print-flow fail closed rather than letting partial/unverified trade samples drive rank.  
**0.3.3 canonical-period rule:** live FY/Q reads collapse same-end-date legacy identities to one canonical evidence row; SEC normalizer `0.3.3-production-data-truth-r3` quarantines stale siblings as `SUPERSEDED_*` while preserving raw facts/provenance/audit. Fundamentals, Current Financial Anatomy, Research basis/readiness, recalculation, Financial Flows, Reports/Publications and valuation forensics now share the canonical period path.  
**0.3.3 Tape truth rule:** Large/Whale values are explicitly dollar notionals. Only current-method, complete consolidated-SIP regular-session observations that filter non-directional trade conditions and reconcile against same-feed daily volume/notional may enter Tape score/regime. Legacy, partial, IEX-only or sanity-failed observations are withheld and old Tape caches are forced stale.  
**0.3.3 production-data-truth pull request / merge:** #67 `0.3.3: Production data truth hardening` passed final PR CI #1790 / run `35746437219` SUCCESS at `a68ff31162cab22dcb0fbf69a0b75ca2819987d7`, then merged by squash to `main` as `872bfc12f3b7d83b0dde1323def81cf10838de7b`.  
**0.3.3 production-data-truth verification:** post-merge main CI #1791 / run `35746602106` SUCCESS. Release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production remains on the previously deployed 0.3.3 source until the next manual deploy.  
**0.3.3 financial-basis integrity R2 branch:** `fix/0.3.3-financial-basis-integrity-r2`, created directly from clean production-synced 0.3.3 main `a55ca938e0fc0a5b2eb1c3059c7d95e12b40d879`; VERSION remains 0.3.3. This pass fixes 52/53-week fiscal-year drift that can create false history holes, repairs corrected period labels in place, makes the freshest filed basis authoritative over stale/same-date TTM, removes hardcoded historical release labels from user-facing Research UI, and bumps the SEC normalizer so existing Coverage self-heals rather than requiring ticker-by-ticker exceptions.  
**0.3.3 current-basis integrity rule:** current Financial Anatomy / Fundamentals strip / downstream current-row consumers choose the latest end-date evidence; a newer FY beats an older TTM and FY wins a same-date year-end tie.  
**0.3.3 fiscal-calendar integrity rule:** current SEC fiscalYearEnd MMDD is tolerant around historical 52/53-week year ends; small week-based drift may not shift the represented fiscal year. Existing same-end-date period labels are repaired in place on re-ingest.  
**0.3.3 UI version rule:** normal user-facing templates may not contain hardcoded semantic release literals; current product version, when intentionally displayed, must come from runtime VERSION.  
**0.3.3 financial-basis integrity R2 pull request / merge:** #66 `0.3.3: Financial basis integrity R2` passed final PR CI #1762 / run `35739117972` SUCCESS at `e0c520e7689af4cc5a2ed915315b7cc1f489653c`, then merged by squash to `main` as `aa7f773159371b362addc8192f61e354dbdc2c1b`.  
**0.3.3 financial-basis integrity R2 verification:** post-merge main CI #1763 / run `35740681009` SUCCESS. Release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production remains on the prior deployed 0.3.3 source until the next manual deploy.  
...
**0.3.3 financial-completeness branch:** `release/0.3.3-financial-completeness`, created from clean 0.3.3 main `129b87975acea7c7d63b537c3b1209d4e79b63ac`; VERSION remains 0.3.3.  
**0.3.3 field-resolution rule:** no symbol-specific financial parser exceptions. The common resolver uses canonical Companyfacts tags, exact semantic labels, conservative accounting algebra, bounded filing-level iXBRL extension facts, same-date FY/Q4 instant bridges and optional secondary-provider fallback in that order. Filing extension facts must be whole-entity/non-dimensional and exact-label matched. Operating Income may be derived from Gross Profit and true Operating Expenses; SGA may stand in for total Operating Expenses only when the candidate independently reconciles to pretax/non-operating evidence and the missing bridge is not structurally large.  
**0.3.3 balance-sheet continuity rule:** a synthetic Q4 inherits same-date FY-end instant facts such as Inventory/Receivables/Cash/Payables/Assets/Liabilities/Equity with explicit `FY_END_INSTANT_BRIDGE` provenance. Current-basis fields that are demonstrably applicable but unresolved open `MISSING_EXPECTED_*` data-quality issues instead of disappearing silently.  
**0.3.3 share-basis correctness:** quarterly diluted-share averages are reconstructed from YTD/FY weighted averages by period length; YTD and FY averages are never used directly as quarter-only proxies. TTM field-level provenance now survives aggregation. Internal SEC normalizer revision is now `0.3.3-financial-basis-integrity-r2`, forcing already-ingested 0.3.3 Coverage to self-heal on the corrected fiscal-calendar/current-basis parser.  
**0.3.3 bulk repair rule:** `Refresh stale` compares each Coverage's latest SEC COMPANYFACTS source to `SEC_NORMALIZER_VERSION`; a normalizer mismatch queues SEC_INGEST even when the prior SEC job is less than 24 hours old. This repairs the entire Coverage set without per-ticker navigation.  
**0.3.3 financial-completeness pull request / merge:** #65 `0.3.3: Financial completeness hardening`; final PR-head CI #1747 / run `35733152767` SUCCESS at `0c09073b54262b77c091bd3b4831c45a1a1e1acf`, then merged by squash to `main` as `4928036b3ae6860ee73532766885c95cdbbe557c`.  
**0.3.3 financial-completeness verification:** branch CI #1744 / run `35732790136` SUCCESS; initial PR CI #1745 / run `35732967339` SUCCESS; final PR-head CI #1747 / run `35733152767` SUCCESS; post-merge main CI #1748 / run `35733336802` SUCCESS. All passed release metadata, Python/YAML/JS checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke. Production remains unchanged until a verified manual deploy updates the top-level `Production:` line.  
...
**0.3.3 existing-Coverage recovery:** SEC COMPANYFACTS sources carry the current internal normalizer revision (`0.3.3-financial-basis-integrity-r2`). Opening Coverage with an older stored normalizer queues one deduplicated background SEC_INGEST; GET navigation itself remains provider-free. Successful SEC ingest recalculates flows/autofill/cache so Current Financial Anatomy, Financial Flows and Expectations share the recovered basis.  
...
**0.3.3 pull request / merge:** #64 `0.3.3: Core financial data recovery` passed final PR CI #1717 / run `35728907466` SUCCESS at `950b845966a97f9ba8a0af809b0c749f9d44f995`, then merged by squash to `main` as `9a04092e8376d9438634ca7716fe22f4cfdb9933`.  
**0.3.3 verification:** branch CI #1714 / run `35728675722` SUCCESS; final PR CI #1717 / run `35728907466` SUCCESS; post-merge main CI #1718 / run `35729104805` SUCCESS. All passed release metadata, Python/YAML/JS checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke. Production remains on the top-level `Production:` line until a verified manual deploy updates it.  
...
**Historical production baseline (0.3.4):** 0.3.4 on Namecheap; verified production health after manual deploy run `35759706905` / deploy #69 from main source `778648888cfff0ca2be2d2d6b003e6fedbb3456c` at 2026-09-22T17:20:05Z; /health matched VERSION and returned `architecture=web-native`, `database=primary`, `reports=rich`, `status=ok`.  
