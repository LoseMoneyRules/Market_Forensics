...
**0.3.5 branch:** `release/0.3.5-data-surface-coherence`, created directly from production-synced 0.3.4 `main` `6461fcd0579c9a8626a2749649a9cc6cd38206c2`; this is a clean material release, not a patch-on-patch branch.  
**0.3.5 data-surface coherence rule:** one represented financial date may have audit-preserved sibling identities, but live Current Financial Anatomy/Fundamentals reads merge only missing normalized fields from siblings with that exact end date and retain field provenance. Values are never carried across dates. This prevents a valid Inventory/Receivables/etc. fact from being visible in history while disappearing from the current strip.  
**0.3.5 materialization rule:** `RECALCULATE` is again the canonical boundary that rebuilds `RESEARCH_CACHE`. Updating underlying facts without regenerating the cache is invalid because Tape, Fundamentals Forensics, Decision Lenses and overview surfaces otherwise remain stale.  
**0.3.5 Tape rule:** one Positioning refresh also repairs the stored HistoricalPrice spine required by Tape charts. Current-method SIP observations that pass factual reconciliation remain visible even when direction-eligible coverage is too low for scoring; low coverage blocks rank/regime influence instead of deleting the observation.  
**0.3.4 branch:** `release/0.3.4-data-surface-integrity`, created directly from production-synced main `0d15f9fcb3264c0f3eb116d9a4af38483d870e54`. This release consolidates the financial current-basis and Tape regressions reported after deploy #68; it is not a patch-on-patch release.  
**0.3.4 financial current-basis rule:** represented-period reads consider audit-preserved same-end `SUPERSEDED_*` / `SUP_*` evidence and choose the richest factual identity rather than the newest active shell. SEC normalizer `0.3.4-data-surface-integrity-r1` reactivates the richest family member on refresh. `current_row()` retains the prior FY when calculating current derived metrics, so Revenue Growth does not disappear merely because the UI requests one current row.  
**0.3.4 Tape rule:** Large/Whale values remain dollar-notional trade-size proxies. Current-method reconciled consolidated-SIP samples remain visible in charts/tables even when partial; sampled notionals are labeled as samples and confidence is coverage-weighted. Partial/page-capped samples are context-only and never influence Tape rank/regime because the bounded beginning/end sample is not a representative whole-day tape. Only complete integrity-passing SIP observations with sufficient daily-volume reconciliation may score. Legacy/IEX/impossible-volume evidence remains withheld.  
**0.3.4 pull request / merge:** PR #68 `0.3.4: Financial & Tape data-surface integrity` passed branch CI #1828 / run `35758056190` and PR CI #1829 / run `35758130036` at `5768e70346ad98bd67fd1ceca2192abfe845fc02`, then merged by squash to `main` as `a154f45276363a60e658337b7f486146190956f6`.  
**0.3.4 verification:** post-merge main CI #1830 / run `35758299401` SUCCESS. Release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production remains 0.3.3 until a separate manual deploy succeeds.  
