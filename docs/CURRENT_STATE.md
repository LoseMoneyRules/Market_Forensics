...
**Production:** 0.3.0 on Namecheap; verified production health after manual deploy run `35611536246` / deploy #56 from main source `293b05ae4572ac5bea0f6ac93f6e42a782d6af66` at 2026-09-21T14:25:19Z; /health matched VERSION and returned `architecture=web-native`, `database=primary`, `reports=rich`, `status=ok`.  
...
**0.3.0 global UI coherence hardening:** `fix/0.3.0-global-ui-coherence` is presentation-only and starts from current 0.3.0 main. It establishes a site-wide 13 px minimum canonical UI/chart label floor, one semantic status palette (positive green, negative red, watch/review amber, neutral gray, informational blue) in both light and dark mode, restructures Tape context cards for readable hierarchy, and adds a compact ticker + latest-price topbar identity that appears after the canonical company header scrolls away. All application templates were audited for inline visual overrides and purple usage; none were found. No analytical engine, database, provider, auth, Portfolio, publication or report logic changes.  
