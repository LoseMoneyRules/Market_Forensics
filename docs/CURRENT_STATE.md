...
**0.3.6 post-merge verification:** main CI run `35783040423` SUCCESS at `343b3f30d29a8a38e07b8a58e15a30b5a83901e6`; release metadata, Python/YAML/JavaScript checks, full pytest, production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. Production remains on the prior deployed 0.3.4 until a separate manual deploy succeeds.  
**Production:** 0.3.4, currently on the prior 0.3.4 deploy from workflow run `35759706905` / production-state sync `6461fcd0579c9a8626a2749649a9cc6cd38206c2`. The 0.3.4 integrity-closure code is merged and verified on main but requires a new manual deploy before production serves it.  
...
**Production:** 0.3.4 on Namecheap; verified production health after manual deploy run `35759706905` / deploy #69 from main source `778648888cfff0ca2be2d2d6b003e6fedbb3456c` at 2026-09-22T17:20:05Z; /health matched VERSION and returned `architecture=web-native`, `database=primary`, `reports=rich`, `status=ok`.  
