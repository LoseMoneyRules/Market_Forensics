...
**Production:** 0.3.4, deployed successfully by workflow run `35759706905`; production-state sync is `6461fcd0579c9a8626a2749649a9cc6cd38206c2`.  
**0.3.6 branch:** `release/0.3.6-integrity-consolidation`, created directly from current `main` `361a08a31d8319424aead22c1b156f1fbb889ae3`. This release supersedes the parallel unmerged 0.3.5 attempt and consolidates the remaining financial-current and Tape fixes into one architecture.  
**0.3.6 financial evidence rule:** live FY/Q reads keep one durable period identity for writes/audit but coalesce normalized fields across exact-same-end-date active / `SUPERSEDED_*` / `SUP_*` siblings. Recovered fields retain explicit `SAME_PERIOD_EVIDENCE_COALESCE` provenance and source period id; period-level Economic Reality may also be recovered from a same-period sibling. No value is ever carried across dates. Current Financial Anatomy, the Fundamentals current strip, recalculation/Financial Flows and downstream Research therefore consume the same represented-period evidence.  
**0.3.6 Research cache rule:** `RECALCULATE` is the single canonical builder of `RESEARCH_CACHE`. Provider/evidence jobs persist evidence and queue recalculation; they do not partially mutate the Research cache. Normal GET navigation still makes no provider calls. Tape may rebuild only its display surface from fresher materialized DB evidence while the canonical cache is stale/pending.  
**0.3.6 Tape rule:** an explicit Tape / Positioning refresh also queues the HistoricalPrice spine required by Tape charts. Valid reconciled consolidated-SIP observations remain visible when direction-eligible coverage is low, but low coverage is a decision/scoring restriction and cannot move Tape rank/regime. Partial samples remain context-only. The flow method version is `0.3.6-reconciled-observation-v2`, forcing incompatible prior Tape cache evidence stale.  
**0.3.5 status:** PR #70 `0.3.5: Data surface coherence and Tape recovery` was created from an older 0.3.4 base and is superseded by 0.3.6; it must not be merged. Its valid ideas were re-applied selectively on top of current `main`.  
...
**0.3.4 integrity-closure verification:** code + page-level regression branch CI #1888 / run `35779575156` SUCCESS at `dde45e84a42451efd37dfcac6869921009ef1866`. Release metadata, Python/YAML/JavaScript checks, full pytest (including populated Fundamentals/Tape rendering), production-minimal startup + rich-report smoke and self-contained reporting-vendor smoke all passed. PR / merge / post-merge verification will be appended before deployment.  
