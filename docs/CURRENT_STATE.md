...
**0.3.0 spacing-rhythm hardening:** `fix/0.3.0-spacing-rhythm` is presentation-only. It restores the missing 18 px section gap between Tape change cards and Tape support KPIs, defines the previously unstylized `form-stack` with a 14 px field/action gap across login/invite/2FA/verify, normalizes legacy auth/control buttons to the canonical button component, and adds consistent spacing for invite-copy and error actions. A full template/CSS spacing audit found the existing canonical panel/table/KPI/action-bar rhythm intact; no analytical engine, database, provider, auth flow, Portfolio, publication or report behavior changes. VERSION remains 0.3.0.  
